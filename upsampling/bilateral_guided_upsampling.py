...
def bgu_fit(input_image: torch.Tensor, guide_image: torch.Tensor, output_image: torch.Tensor,
    ...
    w_stack = torch.stack([(1 - wx_flat) * (1 - wy_flat) * (1 - wz_flat),
                           ...
                           wx_flat * wy_flat * wz_flat], dim=-1)  # w000  # w001  # w010  # w011  # w100  # w101  # w110  # w111
    ...
    try:
        ...
        gamma = bgu_fit(input_image, guide_image, output_image, weight_image, grid_size, reg_lambda, reg_T, use_float64=True)
...
class BGU(nn.Module):
    ...
    """
    def __init__(self, grid_size: Optional[Tuple] = None, reg_lambda: Optional[float] = 1e-7, reg_T: Optional[bool] = True):
        """
        Args:
            grid_size: Tuple (gh, gw, gd, out_ch, C+1). If None, defaults to [round(H/16), round(W/16), 16, out_ch, C+1].
    ...
    def forward(self, high_res_input: torch.Tensor, low_res_input: torch.Tensor, low_res_target: torch.Tensor) -> torch.Tensor:
        """
        Two steps:
        ...
        grid = bgu_fit(low_res_input, low_res_guide, low_res_target, grid_size=self.grid_size, reg_lambda=self.reg_lambda, reg_T=self.reg_T)  # B, gh, gw, gd, 3, 4
